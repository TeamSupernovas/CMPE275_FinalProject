...
int main(int argc, char *argv[]) {
   ...
   QString serverIp = settings.value("SERVER_IP", "192.168.1.102").toString();
    //QString serverIp = settings.value("SERVER_IP", "10.0.0.228").toString();
    qDebug() << serverIp << "\n";
...
QTcpSocket clientSocket;
QMap<QString, NodeInfo> nodeMap; // Map to store node information by IP address
int myNumber; // Store the generated number for this node
QString myIp;
...
int main(int argc, char *argv[]) {
    ...
    int port = settings.value("PORT", 12350).toInt(); // Default port if not specified
